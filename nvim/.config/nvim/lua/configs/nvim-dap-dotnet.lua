...
-- https://www.youtube.com/watch?v=CMZlSYYdSmU

local M = {}

function M.find_project_by_csproj(start_path)
...
function M.get_highest_net_folder(bin_debug_path)
  ...
  if dirs == 0 then
    error("No .NET folders found in " .. bin_debug_path)
  end
...
function M.build_dll_path()
  ...
  local project_root = M.find_project_by_csproj(current_dir)
  ...
  if #csproj_files == 0 then
    error("No .csproj files found in project root: " .. project_root)
  end
